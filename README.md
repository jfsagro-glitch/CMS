...
- Для production-запросов к DeepSeek требуется серверный прокси (браузер блокирует прямые запросы по CORS).
- Рекомендуемый вариант — Cloudflare Worker из папки `proxy/`. Подробная инструкция: `proxy/README.md`.
- После деплоя прокси укажите его URL в переменной `VITE_DEEPSEEK_PROXY_URL` (например, в `.env.production`).
- В development (localhost) Vite-прокси продолжит работать автоматически.
