...
function inferMainLabelKey(keys) {
  ...
  // Fallback to the first non-empty key
...
function main() {
  ...
  for (const file of files) {
    try {
      ...
      dictionaries.push({
        ...
        level: 1,
        dependencyHints, // possible attribute links
        ...dict,
      });
    } catch (e) {
  ...
  // Unified attribute list (each dictionary -> an attribute definition)
  const unifiedAttributes = dictionaries.map(d => ({
    ...
    dependencyHints: d.dependencyHints?.map(h => ({ id: h.id, code: h.code, name: h.name })) || [],
  }));
...
// Export for Node REPL/manual invocation
module.exports = { main };
