...
import {
  getAppraisalConfigForType,
  formatAppraisalAttributes,
...
const MessageItem: React.FC<MessageItemProps> = React.memo(
  ({ message, onRating, onTopicClick }) => {
    ...
    return (
      <div className={`reference-page__message reference-page__message--${message.role}`}>
        <div className="reference-page__message-content">
          ...
          <div className="reference-page__message-text">
            <div className="reference-page__message-header">
              <Text strong>{message.role === 'user' ? 'Вы' : 'ИИ Помощник'}</Text>
...
const ReferencePage: React.FC = () => {
  ...
  interface ExpertiseSkills {
    ...
    useDeepSeek?: boolean; // Использовать AI DeepSeek для ответов
  }
      ...
      if (typeOption) {
        setAppraisalSkills(prevSkills => {
          ...
          if (JSON.stringify(updatedSkills) !== JSON.stringify(prevSkills)) {
            message.info(
              `Настройки эквалайзера автоматически применены для типа: ${typeOption.label}`
            );
  ...
  const buildAppraisalContext = useCallback(() => {
    ...
    const parts: string[] = [];
    const categoryLabel =
      appraisalCategoryOptions.find(opt => opt.value === (values.assetGroup || derivedAssetGroup))
        ?.label ||
      values.assetGroup ||
      derivedAssetGroup;
    ...
    const runTrainingCycle = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          ...
          setLearningIndex(
            Math.round(
              patternsWeight + successWeight + usageWeight + insightsWeight + evolutionBonus
            )
          );
  ...
  const searchCacheRef = useRef<Map<string, { result: KnowledgeTopic[]; timestamp: number }>>(
    new Map()
  );
  ...
  const RobotIcon = ({ size = robotSize }: { size?: number }) => (
    ...
    >
      ...
      {/* Рот/индикатор */}
      <rect x="236" y="240" width="40" height="8" rx="4" fill="#1890ff" />
      {/* Антенна */}
  ...
  useEffect(() => {
    ...
    const loadDocuments = async () => {
      ...
      try {
        ...
        await Promise.all([documentIndexer.loadFromStorage(), knowledgeBase.loadFromStorage()]);
        ...
        const loadDocumentsAsync = async () => {
          try {
            ...
            if (documents.length > 0 && isMounted) {
              message.success(
                `Загружено документов: ${documents.length}. База знаний готова к использованию.`
              );
            }
          } catch (error) {
            if (isMounted) {
              ...
              message.warning(
                'Не удалось загрузить документы из VND. Вы можете загрузить их вручную.'
              );
            }
          } finally {
            if (isMounted) {
  ...
  const generateAIResponse = useCallback(
    async (
      ...
      chatHistory: Message[] = [],
      extraContext?: string
    ): Promise<{ content: string; sources: KnowledgeTopic[]; context: string }> => {
      ...
      try {
        ...
        const useDeepSeek = expertiseSkills.useDeepSeek !== false;
        if (chatHistory.length > 0) {
          ...
          let systemContext =
            knowledgeContext ||
            `База знаний содержит информацию о банковских залогах, ипотеке, оценке имущества, LTV, договорах залога, нормативных требованиях и регистрации залогов. Доступные категории: ${categoryNames}.`;

          // Добавляем настройки эквалайзера для режима экспертизы и профиль DeepSeek
...
- Использовать AI DeepSeek для генерации ответов: ${useDeepSeek ? 'ДА' : 'НЕТ'}
...
Учитывай эти настройки при формировании ответа.`;
          response = await deepSeekService.generateResponse(
            userMessage,
            knowledgeContext + expertiseSettings
          );
          ...
          if (lowerMessage.includes('привет') || lowerMessage.includes('здравствуй')) {
            const greetingContext =
              'Поприветствуй пользователя и расскажи, что ты эксперт по банковским залогам и можешь помочь с вопросами об ипотеке, оценке, LTV, договорах залога и других аспектах залогового кредитования. База знаний основана на справочной литературе "Залоговik. Все о банковских залогах".';
            const useDeepSeek = expertiseSkills.useDeepSeek !== false;
            const expertiseSettings = `
\n\nНастройки режима экспертизы (эквалайзер):
- Уровень экспертизы: ${expertiseSkills.expertiseLevel}%
...
Учитывай эти настройки при формировании ответа.`;
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      if (
        ...
        errorContent +=
          'Проблема с подключением к интернету. Проверьте соединение и попробуйте еще раз.';
      } else if (errorMessage.includes('timeout') || errorMessage.includes('время ожидания')) {
        errorContent +=
          'Превышено время ожидания ответа (30 секунд). Попробуйте еще раз или упростите вопрос.';
        ...
        errorContent += `Попробуйте еще раз или переформулируйте вопрос. (Ошибка: ${errorMessage.substring(
          0,
          100
        )})`;
      }
      ...
      message.error(
        'Не удалось получить ответ от AI помощника. Проверьте консоль браузера для деталей.'
      );
  ...
  const handleRating = useCallback(
    (messageId: string, rating: 'like' | 'dislike') => {
      ...
      try {
        ...
        const category = msg.context?.toLowerCase().includes('ltv')
          ? 'ltv_calculation'
          : msg.context?.toLowerCase().includes('оценк')
          ? 'appraisal'
          : msg.context?.toLowerCase().includes('риск')
          ? 'risks'
          : 'general';
        ...
        if (currentLevel && evolutionStats && evolutionStats.level > 1) {
          ...
          if (levelData) {
            message.success(
              `🎉 Модель достигла уровня: ${levelData.name}! Опыт: ${evolutionStats.totalExperience}`
            );
          }
        }
  ...
  const handleFileUpload = useCallback(async (file: File) => {
    ...
    if (!isSupported) {
      message.error(
        'Поддерживаются только файлы: PDF, DOCX, XLSX, XLS, JPG, JPEG, PNG, GIF, BMP, WEBP'
      );
      return false;
    }
  ...
  const handleAppraisalImageUpload = useCallback(async (file: File) => {
    const fileName = file.name.toLowerCase();
    const isImage = ['.jpg', '.jpeg', '.png', '.gif', '.bmp', '.webp'].some(ext =>
      fileName.endsWith(ext)
    );
    if (!isImage) {
      message.error(
        'Для иллюстрации объекта поддерживаются только изображения (JPG, PNG, GIF, BMP, WEBP)'
      );
  ...
  const handleGenerateAppraisalEstimate = useCallback(async () => {
    try {
      ...
      const combinedAdditionalFactors = [values.additionalFactors, attributeSummary]
        .filter(Boolean)
        .join('. ');
      const locationHint =
        attributes.location || attributes.baseLocation || attributes.storageLocation || undefined;
      const areaHint =
      ...
      if (!appraisalImageData) {
        try {
          const objectDescription = `${
            values.objectName || typeOption?.label || 'Объект оценки'
          }. ${typeOption?.label || ''}. ${locationHint ? `Расположен: ${locationHint}.` : ''} ${
            conditionHint ? `Состояние: ${conditionHint}.` : ''
          } ${attributeSummary ? `Характеристики: ${attributeSummary}.` : ''}`;
  ...
  const handleExportAppraisalPdf = useCallback(async () => {
    ...
    try {
      ...
      const typeOption = appraisalTypeOptions.find(opt => opt.value === values.assetType);
      const objectName =
        (values.objectName as string | undefined)?.trim() || typeOption?.label || 'Объект оценки';
      const location =
        attributes.location || attributes.baseLocation || attributes.storageLocation || '';
      ...
      if (!imageDataToUse) {
        try {
          ...
          const attributeSummary = formatAppraisalAttributes(attributes, attributeFields);
          const objectDescription = `${objectName}. ${typeOption?.label || ''}. ${
            location ? `Расположен: ${location}.` : ''
          } ${attributeSummary ? `Характеристики: ${attributeSummary}.` : ''}`;
      ...
      // Обрабатываем аналоги — формируем подробное текстовое описание (без загрузки внешних изображений)
      let comparablesHtml =
        ...
        const comparablesItems = await Promise.all(
          appraisalEstimate.comparables.map(async (c, idx) => {
            ...
            const finalUrlLink = finalUrl
              ? ` <br/><span style="font-size: 10px;">Ссылка на источник: <a href="${finalUrl}" target="_blank" style="color: #1890ff; text-decoration: underline; word-break: break-all;">${finalUrl}</a></span>`
              : '';

            // Дополнительное текстовое описание по аналогу (без попытки загрузить изображение)
            const detailsText =
              (comparable as any).details ||
              (comparable as any).additionalInfo ||
              (comparable as any).notes ||
              '';

            return `
              <div style="margin: 0 0 12px 0; padding: 8px; background: #f5f5f5; border-radius: 4px; border-left: 3px solid #1890ff;">
                <p style="margin: 0 0 4px 0;"><strong>Аналог ${idx + 1}:</strong> ${description}</p>
                ${
                  detailsText
                    ? `<p style="margin: 0 0 4px 0; font-size: 10px; color: #555;">Дополнительные характеристики: ${detailsText}</p>`
                    : ''
                }
                ${finalUrlLink}
              </div>
        ...
        <div style="font-family: system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; padding: 16px; font-size: 11px; line-height: 1.4; width: 750px; box-sizing: border-box; background:#ffffff;">
          ...
          <div style="display: flex; align-items: center; margin: 8px 0 12px 0;">
            ...
            <div style="font-size: 10px;">
              <div><strong>Тип актива:</strong> ${
                typeOption?.label || values.assetType || '-'
              }</div>
              ${location ? `<div><strong>Локация:</strong> ${location}</div>` : ''}
            </div>
          </div>
          ...
          <pre style="white-space: pre-wrap; margin: 0 0 8px 0;">${
            contextText || 'Нет данных'
          }</pre>

          <h3 style="margin: 12px 0 4px 0;">2. Итоги экспресс-оценки</h3>
          <ul style="margin: 0 0 8px 20px; padding: 0;">
            <li>Рыночная стоимость (ориентировочно): ${appraisalEstimate.marketValue.toLocaleString(
              'ru-RU'
            )} ₽</li>
            <li>Залоговая стоимость (ориентировочно): ${appraisalEstimate.collateralValue.toLocaleString(
              'ru-RU'
            )} ₽</li>
            ${
  ...
  const handleReindexAll = useCallback(async () => {
    ...
    try {
      ...
      message.success(
        `Переиндексация завершена. Обработано документов: ${documents.length}. Категорий: ${updatedCategories.length}. Данные для самообучения обновлены.`
      );
    } catch (error) {
      if (import.meta.env.MODE === 'development') {
          ...
          generateAIResponse(
            ...
            .catch(error => {
              ...
              const errorResponse: Message = {
                ...
                role: 'assistant',
                content:
                  'Извините, произошла ошибка при генерации ответа. Попробуйте еще раз или переформулируйте вопрос.',
                timestamp: new Date(),
              };
              setMessages(prev => [...prev, errorResponse]);
            })
    ...
    () => [
      {
        ...
        query:
          'Как правильно рассчитать LTV (loan-to-value) для залогового имущества? Какие факторы влияют на залоговую стоимость?',
      },
      {
        icon: <FileTextOutlined />,
        text: 'Оценка залогового имущества',
        query:
          'Какие требования к независимой оценке залогового имущества? Как выбрать оценщика и проверить отчет об оценке?',
      },
      {
        icon: <BulbOutlined />,
        text: 'Анализ рисков залога',
        query:
          'Какие основные риски при принятии имущества в залог? Как оценить и минимизировать риски по залоговому обеспечению?',
      },
      {
        icon: <QuestionCircleOutlined />,
        text: 'Регистрация обременения в Росреестре',
        query:
          'Как зарегистрировать обременение в Росреестре? Какие документы необходимы для регистрации залога недвижимости?',
      },
      {
        icon: <FileTextOutlined />,
        text: 'Визуальный осмотр залогового имущества',
        query:
          'Как провести визуальный осмотр залогового имущества? На что обратить внимание при проверке наличия и состояния залога?',
      },
      {
        icon: <BulbOutlined />,
        text: 'Обращение взыскания на залог',
        query:
          'В каких случаях можно обратить взыскание на залоговое имущество? Каков порядок реализации залога?',
      },
    ],
    []
  );
  ...
  return (
    <div className="reference-page">
      ...
      <div
        className={`reference-page__sidebar ${
          chatsVisible ? 'reference-page__sidebar--visible' : ''
        }`}
      >
                ...
                {groupedChats.today.length > 0 && (
                  <div className="reference-page__chats-group">
                    ...
                    {groupedChats.today.map(chat => (
                      <div
                        key={chat.id}
                        className={`reference-page__chat-item ${
                          currentChatId === chat.id ? 'reference-page__chat-item--active' : ''
                        }`}
      ...
      {/* Основной контент */}
      <div
        className={`reference-page__main-content ${
          chatsVisible ? 'reference-page__main-content--with-sidebar' : ''
        }`}
      >
        <div className="reference-page__header">
          <div className="reference-page__header-left">
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <Space align="center" style={{ marginBottom: 4 }} size="small">
                ...
                <Tag
                  color={
                    learningIndex >= 70 ? 'success' : learningIndex >= 40 ? 'processing' : 'default'
                  }
                  ...
                  <Space direction="vertical" size={2} style={{ width: '100%' }}>
                    <Space size="small">
                      <Text strong style={{ fontSize: '12px' }}>
                        Уровень: {evolutionService.getCurrentLevel()?.name || 'Новичок'} (
                        {evolutionLevel})
                      </Text>
                    ...
                    <Progress
                      ...
                      format={() =>
                        `${evolutionProgress.current}/${evolutionProgress.required} опыта`
                      }
                      size="small"
                      style={{ marginTop: 2 }}
                      showInfo={false}
          ...
          <div className="reference-page__header-right">
            ...
            >
              Чаты{' '}
              {chats.length > 0 && (
                <Badge count={chats.length} size="small" style={{ marginLeft: 4 }} />
              )}
            </Button>
            <Button
              icon={<SettingOutlined />}
              ...
              <Space direction="vertical" style={{ width: '100%' }} size="middle">
                ...
                <Space size="middle" wrap>
                  <Form.Item label="Категория" name="assetGroup">
                    <Select style={{ minWidth: 220 }} options={appraisalCategoryOptions} disabled />
                  </Form.Item>
                  <Form.Item
                    label="Тип актива"
                    name="assetType"
                ...
                <Space style={{ justifyContent: 'space-between', width: '100%' }}>
                  <Text type="secondary">
                    После заполнения полей AI помощник выполнит экспресс-оценку и учтёт данные в
                    самообучении.
                  </Text>
                  <Space>
                    <Button
                ...
                {appraisalEstimate && (
                  <Alert
                    type="success"
                    showIcon
                    message={`Рыночная стоимость: ${appraisalEstimate.marketValue.toLocaleString(
                      'ru-RU'
                    )} ₽ · Залоговая: ${appraisalEstimate.collateralValue.toLocaleString(
                      'ru-RU'
                    )} ₽`}
                    description={
                      <div>
                        ...
                        <br />
                        <Text strong>Ключевые риски:</Text>{' '}
                        {appraisalEstimate.riskFactors.join('; ') || 'не выявлены'}
                        <br />
                        <Text strong>Рекомендации:</Text>{' '}
                        {appraisalEstimate.recommendedActions.join('; ') || '—'}
                        <br />
                ...
                description={
                  <div>
                    ...
                    {typeof loadingCountdown === 'number' && (
                      <div style={{ marginTop: 4 }}>Осталось примерно {loadingCountdown} сек.</div>
                    )}
                  </div>
                }
                type="info"
                    ...
                    <div className="reference-page__input-wrapper">
                      <div style={{ position: 'relative', width: '100%' }}>
                        <TextArea
                          ...
                          placeholder={
                            messages.length === 0
                              ? 'Задайте вопрос о залогах, ипотеке, оценке...'
          ...
          <Space direction="vertical" style={{ width: '100%' }} size="large">
            ...
            <div>
              <Space style={{ marginBottom: 8 }}>
                ...
                {selectedCategory && (
                  <Button type="link" size="small" onClick={() => handleCategorySelect(null)}>
                    Сбросить
                  </Button>
                )}
              </Space>
            ...
            {searchResults.length > 0 && !selectedCategory && (
              <>
                <Divider style={{ margin: '8px 0' }} />
                <div>
                  <Text strong>Результаты поиска ({searchResults.length})</Text>
                  <List
                    size="small"
        ...
        >
          <Space direction="vertical" size="large" style={{ width: '100%' }}>
              ...
              <Space direction="vertical" size="middle" style={{ width: '100%' }}>
                <div>
                  ...
                  <Slider
                    ...
                    onChange={value =>
                      setAppraisalSkills(prev => ({ ...prev, incomeApproach: value }))
                    }
                    min={0}
                    max={100}
                ...
                <div>
                  ...
                  <Slider
                    ...
                    onChange={value =>
                      setAppraisalSkills(prev => ({ ...prev, comparativeApproach: value }))
                    }
                    min={0}
                    max={100}
                    marks={{ 0: '0%', 50: '50%', 100: '100%' }}
                ...
                <div>
                  ...
                  <Slider
                    ...
                    onChange={value =>
                      setAppraisalSkills(prev => ({ ...prev, costApproach: value }))
                    }
                    min={0}
                    max={100}
                    marks={{ 0: '0%', 50: '50%', 100: '100%' }}
            ...
            <div>
              ...
              <Text type="secondary">
                В этом режиме ответы формируются профессиональным AI помощником (DeepSeek) на основе
                базы знаний и настроек эквалайзера скилов и навыков.
              </Text>
              <Space direction="vertical" size="middle" style={{ width: '100%' }}>
                <div>
                  <Space>
                    <Text strong>Использовать AI DeepSeek для ответов</Text>
                    <Switch
                      checked={expertiseSkills.useDeepSeek !== false}
                      onChange={checked =>
                        setExpertiseSkills(prev => ({ ...prev, useDeepSeek: checked }))
                      }
                    />
                  </Space>
                  <Text type="secondary" style={{ display: 'block', marginTop: 4 }}>
                    При включении этого параметра ИИ будет отвечать как профессионал в области
                    оценки и рисков залогового имущества Банка, опираясь на базу знаний и настройки
                    эквалайзера.
                  </Text>
                </div>
                <div>
                  ...
                  <Slider
                    ...
                    onChange={value =>
                      setExpertiseSkills(prev => ({ ...prev, expertiseLevel: value }))
                    }
                    min={0}
                    max={100}
                    marks={{ 0: '0%', 50: '50%', 100: '100%' }}
                ...
                <div>
                  ...
                  <Slider
                    ...
                    onChange={value =>
                      setExpertiseSkills(prev => ({ ...prev, analysisLevel: value }))
                    }
                    min={0}
                    max={100}
                    marks={{ 0: '0%', 50: '50%', 100: '100%' }}
                ...
                <div>
                  ...
                  <Slider
                    ...
                    onChange={value =>
                      setExpertiseSkills(prev => ({ ...prev, responseBrevity: value }))
                    }
                    min={0}
                    max={100}
                ...
                <div>
                  ...
                  <Slider
                    ...
                    onChange={value =>
                      setExpertiseSkills(prev => ({ ...prev, riskAnalysis: value }))
                    }
                    min={0}
                    max={100}
                    marks={{ 0: '0%', 50: '50%', 100: '100%' }}
                ...
                <div>
                  ...
                  <Slider
                    ...
                    onChange={value =>
                      setExpertiseSkills(prev => ({ ...prev, detailLevel: value }))
                    }
                    min={0}
                    max={100}
                    marks={{ 0: '0%', 50: '50%', 100: '100%' }}
